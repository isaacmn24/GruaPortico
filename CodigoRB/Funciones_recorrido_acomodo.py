# Funciones para leer salidas de modos de acomodo y demÃ¡s
...
def movimientos(c_recorrido, modo_op, M_movimientos, M_master):
    if modo_op == 'Modo Patron':
        ...
        for i in range (len(M_master)): 
            ...
            if coordenadas_f == coordenada_escaneada: # Para que funcione modo patrÃ³n
                color = M_master[i][2]

###    que en modo patrón no aparesca la matriz y que si se vaya creando
    else:
        Recorrido = c_recorrido
        coordenadas_i = M_movimientos[Recorrido+1]
        coordenadas_f = M_movimientos[Recorrido]
